...
27 2 * CONSTANT LCD_ADDRESS

\ void send_nibble(uint8_t nibble, uint8_t rs) {
\     uint8_t data = nibble | (rs ? 0x01 : 0x00); // RS
\     data |= 0x08; // Backlight ON
\     i2c_write(PCF8574_ADDR, data | 0x04); // E=1
...
\     send_nibble(high, 0); // RS = 0 for command
\     send_nibble(low, 0);
\ }

: lcd-send ( n rs -- ) \ send byte n to the LCD (rs is 0 for command, 1 for data)
    >R dup F0 and R@ lcd-send-nibble \ send high nibble
    4 lshift F0 and R> lcd-send-nibble \ send low nibble
    ;


: lcd-send-cmd ( n -- ) \ send a command n (0-F) to the LCD
    0 lcd-send \ rs = 0 for command
    ;


: lcd-send-data ( n -- ) \ send data n to the LCD
...
: lcd-init
    ...
    8 lcd-send-cmd  \ Display off
    1 ms
    1 lcd-send-cmd  \ Clear display
    1 ms            \ Wait for clear
    6 lcd-send-cmd  \ Entry mode set: increment, no shift
    1 ms
    C lcd-send-cmd  \ Display on, cursor off
    1 ms
;
